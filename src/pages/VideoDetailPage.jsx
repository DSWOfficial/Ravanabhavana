...
export default function VideoDetailPage() {
  ...
  const { loading, error, videos } = useVideoLibraryData();
      ...
      <main className="bg-[var(--theme-page)]">
        <section className="section">
          <div className="container-shell">
            ...
            {loading && <p className="surface rounded-lg p-5">Loading video...</p>}
            {!loading && !video && <p className="surface rounded-lg p-5">Video not found.</p>}
            {video && (
              <div className="grid gap-8 lg:grid-cols-[1.35fr_0.65fr]">
                <div>
                  <YouTubeEmbed video={video} />
                  <div className="mt-5 surface rounded-lg p-6">
                    <p className="eyebrow">{video.playlist?.title || 'Uncategorized'}</p>
                    <h1 className="mt-2 text-4xl font-black text-[var(--theme-primary)]">{video.title}</h1>
                    ...
                    <p className="mt-5 whitespace-pre-wrap leading-8 text-[var(--theme-text)]">{video.description}</p>
                    <div className="mt-6 flex flex-wrap gap-2">
                      <button className="btn btn-outline" onClick={() => saved.toggleSavedVideo(video)}><Bookmark size={17} />{saved.savedVideos[video.id] ? 'Saved Video' : 'Save Video'}</button>
                      <button className="btn btn-outline" onClick={() => saved.markWatched(video)}><CheckCircle2 size={17} />{saved.progress[video.id]?.completed ? 'Watched' : 'Mark Watched'}</button>
                      <button className="btn btn-outline" onClick={notes.openNotes}><NotebookPen size={17} />Notes</button>
                      <Link className="btn btn-primary" to={`/videos/playlist/${video.playlist?.slug || 'uncategorized'}`}>Back to Playlist</Link>
                    </div>
                  </div>
                </div>
                <aside className="grid content-start gap-4">
                  <div className="surface rounded-lg p-5">
                    <h2 className="text-xl font-black text-[var(--theme-primary)]">Next in playlist</h2>
                    <div className="mt-4 grid gap-3">
                      {nextVideos.map((item) => <SmallVideoLink key={item.id} video={item} />)}
                      {!nextVideos.length && <p className="text-[var(--theme-muted)]">No next videos yet.</p>}
                    </div>
                  </div>
                  <div className="surface rounded-lg p-5">
                    <h2 className="text-xl font-black text-[var(--theme-primary)]">Related videos</h2>
                    <div className="mt-4 grid gap-3">
                      {related.map((item) => <SmallVideoLink key={item.id} video={item} />)}
                      {!related.length && <p className="text-[var(--theme-muted)]">Related videos will appear here soon.</p>}
                    </div>
                  </div>
                </aside>
                {!!nextVideos.length && <div className="lg:col-span-2"><h2 className="text-2xl font-black text-[var(--theme-primary)]">More from this playlist</h2><div className="mt-4 grid gap-5 md:grid-cols-2 lg:grid-cols-4">{nextVideos.map((item) => <VideoCard key={item.id} video={item} saved={Boolean(saved.savedVideos[item.id])} completed={Boolean(saved.progress[item.id]?.completed)} onSave={saved.toggleSavedVideo} onWatched={saved.markWatched} />)}</div></div>}
              </div>
            )}
          </div>
        </section>
...
function SmallVideoLink({ video }) {
  return (
    <Link className="grid grid-cols-[96px_1fr] gap-3 rounded-lg bg-[var(--theme-section)] p-2 transition hover:bg-[color-mix(in_srgb,var(--theme-accent)_14%,var(--theme-section))]" to={`/videos/${video.slug}`}>
      ...
      <span className="text-sm font-black text-[var(--theme-primary)]">{video.title}</span>
    </Link>
  );
}
