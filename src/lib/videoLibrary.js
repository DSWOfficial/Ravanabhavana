...
export function normalizePlaylist(data = {}, id = data.id) {
  ...
  return {
    ...
    sinhalaTitle: data.sinhalaTitle || data.title_si || '',
    title_si: data.title_si || data.sinhalaTitle || '',
    ...
    courseMode: Boolean(data.courseMode),
...
export function normalizeVideo(data = {}, id = data.id) {
  ...
  return {
    ...
    materials: Array.isArray(data.materials) ? data.materials.map((material, index) => ({
      id: material.id || `material-${index + 1}`,
      title: material.title || 'Resource',
      type: material.type || 'external',
      url: material.url || '',
      mediaId: material.mediaId || '',
      description: material.description || '',
      order: Number(material.order ?? index + 1),
