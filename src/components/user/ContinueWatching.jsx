  ...
  return (
    <section className="surface rounded-lg p-6">
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {list.map((video) => <a key={video.id} href={video.youtubeUrl} target="_blank" rel="noreferrer" className="interactive-card rounded-lg bg-[var(--theme-surface)] p-3"><img className="aspect-video w-full rounded object-cover" src={video.thumbnailUrl} alt={video.title} /><b className="mt-3 block text-[var(--theme-primary)]">{video.title}</b></a>)}
      </div>
