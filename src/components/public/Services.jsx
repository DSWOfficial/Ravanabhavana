  ...
  return (
    <section id="services" className="section bg-[var(--theme-section)]">
      <div className="container-shell">
        ...
        <h2 className="mt-3 text-4xl font-black text-[var(--theme-primary)]">{t('nav.services')}</h2>
          ...
          {services.map((service) => {
            ...
            return (
              <article key={service.id || service.title} className="surface interactive-card rounded-lg p-6">
                ...
                <h3 className="mt-4 text-xl font-black text-[var(--theme-primary)]">{getLocalized(service, 'title', service.title)}</h3>
                <p className="mt-3 leading-7 text-[var(--theme-muted)]">{getLocalized(service, 'description', service.description)}</p>
              </article>
            );
          })}
        </div>
