  ...
  return (
    <section id="guidance" className={`section ${compact ? 'bg-[var(--theme-section)]' : 'bg-[var(--theme-surface)]'}`}>
      <div className="container-shell grid gap-8 lg:grid-cols-[0.95fr_1.05fr]">
        <div>
          <span className="inline-flex items-center gap-2 rounded-full bg-[var(--theme-section)] px-3 py-1 text-sm font-black text-[var(--theme-primary)]"><LockKeyhole size={16} />Anonymous & private</span>
          <h2 className="mt-4 text-4xl font-black text-[var(--theme-primary)]">Need Guidance?</h2>
          <p className="mt-4 max-w-2xl text-lg leading-8 text-[var(--theme-muted)]">Ask anonymously for spiritual guidance, counselling support, life advice, or personal help. Your identity is never shown publicly.</p>
          <div className="mt-8">
            <h3 className="text-2xl font-black text-[var(--theme-primary)]">Guidance Library</h3>
            <div className="mt-4 grid gap-3">
              ...
              {!library.length && <p className="rounded-lg bg-[var(--theme-section)] p-4 font-semibold text-[var(--theme-muted)]">Guidance answers will appear here soon.</p>}
            </div>
          </div>
        </div>
        <form onSubmit={submit} className="surface rounded-lg p-6">
          <select className="input" value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })}>{categories.map((category) => <option key={category}>{category}</option>)}</select>
          <textarea className="input mt-3 min-h-56 text-base" placeholder="Write your question privately..." value={form.question} onChange={(e) => setForm({ ...form, question: e.target.value })} />
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            <input className="input" placeholder="Optional name" value={form.optionalName} onChange={(e) => setForm({ ...form, optionalName: e.target.value })} />
            <input className="input" placeholder="Optional WhatsApp number" value={form.optionalWhatsapp} onChange={(e) => setForm({ ...form, optionalWhatsapp: e.target.value })} />
          </div>
          <label className="mt-4 flex gap-2 font-semibold text-[var(--theme-muted)]"><input type="checkbox" checked={form.wantsPrivateReply} onChange={(e) => setForm({ ...form, wantsPrivateReply: e.target.checked })} />I want a private reply</label>
          <label className="mt-2 flex gap-2 font-semibold text-[var(--theme-muted)]"><input type="checkbox" checked={form.allowAnonymousPublish} onChange={(e) => setForm({ ...form, allowAnonymousPublish: e.target.checked })} />You may publish this anonymously if useful to others</label>
          ...
          <button className="btn btn-primary mt-5 w-full"><Send size={18} />Submit privately</button>
        </form>
      </div>
    </section>
  );
