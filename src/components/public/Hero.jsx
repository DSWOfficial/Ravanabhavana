    ...
    <section id="home" className="relative overflow-hidden bg-[var(--theme-hero)] text-[var(--theme-hero-text)]">
      <div className="container-shell grid min-h-[680px] items-center gap-10 py-16 lg:grid-cols-[1.05fr_0.95fr]">
        <div>
          ...
          <h1 className="mt-4 text-5xl font-black leading-tight sm:text-7xl">{hero.title}</h1>
          <p className="mt-3 text-2xl font-bold text-[color-mix(in_srgb,var(--theme-accent)_76%,white)]">{hero.subtitle}</p>
          <p className="mt-6 max-w-2xl whitespace-pre-wrap text-lg leading-8 text-[color-mix(in_srgb,var(--theme-hero-text)_84%,var(--theme-accent))]">{hero.description}</p>
          <div className="mt-8 flex flex-wrap gap-3">
            <button className="btn btn-gold" onClick={() => openWhatsApp(settings.whatsappNumber, createContactWhatsAppMessage())}><MessageCircle size={19} />{hero.primaryLabel}</button>
            <a className="btn btn-outline border-[color-mix(in_srgb,var(--theme-accent)_55%,transparent)] text-[var(--theme-hero-text)]" href={hero.secondaryUrl || '#videos'}><PlayCircle size={19} />{hero.secondaryLabel}</a>
            <a className="btn btn-outline border-[color-mix(in_srgb,var(--theme-accent)_55%,transparent)] text-[var(--theme-hero-text)]" href="#session"><Video size={19} />Weekly Session</a>
        ...
        <div className="flex justify-center">
          <img src={hero.imageUrl || '/ravana-bhawana-logo.png'} alt={hero.title || 'Ravana Bhavana'} className="w-full max-w-[500px] rounded-full border border-[color-mix(in_srgb,var(--theme-accent)_40%,transparent)] shadow-2xl transition duration-300 hover:scale-[1.025]" />
        </div>
      </div>
      <div className="border-y border-[color-mix(in_srgb,var(--theme-accent)_26%,transparent)] bg-[color-mix(in_srgb,var(--theme-hero)_82%,var(--theme-primary))] py-4">
        <div className="container-shell flex flex-col gap-2 text-sm font-bold sm:flex-row sm:items-center sm:justify-between">
          <span>Send your question as a WhatsApp voice message.</span>
